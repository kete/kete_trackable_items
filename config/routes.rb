...
ActionController::Routing::Routes.draw do |map|
  ...
  map.resources :trackable_item_shelf_locations, :path_prefix => ':urlified_name', :only => [:new, :create, :update], :collection => { :bulk_allocation => [:get], :bulk_import => [:post], :generate_export => [:post] }
